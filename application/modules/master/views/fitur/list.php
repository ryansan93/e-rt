	...
	<tr class="head cursor-p search" title="Klik untuk melihat detail" data-val="0">
		...
		<td class="col-sm-2 text-center">
			<?php if ( $akses['a_edit'] == 1 ): ?>
				<button id="btn-edit" type="button" class="btn btn-primary cursor-p" title="EDIT" onclick="fitur.edit_form(this)"> 
					<i class="fa fa-pencil-square-o" aria-hidden="true"></i> 
				</button>
			<?php endif ?>
			<?php if ( $akses['a_delete'] == 1 ): ?>
				<button id="btn-delete" type="button" class="btn btn-danger cursor-p" title="DELETE" onclick="fitur.delete(this)"> 
			...
			<table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
				<thead>
					<tr>
						...
						<th class="col-sm-7">Path Fitur</th>
					</tr>
				</thead>
				<tbody>
					<?php foreach ($val['detail_fitur'] as $key => $d_val): ?>
						<tr>
							<td class="text-right"><?php echo $d_val['no_urut']; ?></td>
