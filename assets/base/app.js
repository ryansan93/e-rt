var App = {
    ...
    setTutupBulan : function( elm ){
        var s = window.location.href;
        var jenis = null;
        if (s.toLowerCase().includes('transaksi')) {
            jenis = 'transaksi';
        }

        if (s.toLowerCase().includes('accounting')) {
            jenis = 'accounting';
        }
            ...
            success : function(data){
                if ( !empty( data.content ) ) {
                    for (let i = 0; i < elm.length; i++) {
                        const element = elm[i];
                        if ( $(element).length > 0 ) {
                            $(element).data("DateTimePicker").minDate(moment(new Date(data.content.maxDate)));
                        }
                    }

                    var div_btn = $('div.div_tutup_bulan');
                    var val_tgl = $(div_btn).attr('data-val');

                    if ( val_tgl < data.content.maxDate ) {
                        $(div_btn).find('.btn_tutup_bulan').addClass('hide');
                    } else {
                        $(div_btn).find('.btn_tutup_bulan').removeClass('hide');
                    }
                }
            },
        });
