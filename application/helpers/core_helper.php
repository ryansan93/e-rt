...
if (! function_exists ( 'formatAngka' )) {
	function formatAngka($angka, $default = 0) {
    if ( isset( $angka ) ) {
      if ( is_decimal( $angka ) ) {
        $length_decimal = 0;
        if ( $default > 0 ) {
          $length_decimal = $default;
        } else {
          $str = $angka + 0;
          $length_decimal = strlen(substr(strrchr($str, "."), 1));

          $length_decimal = ($length_decimal > 10) ? 10 : $length_decimal;
        }

        return angkaDecimalFormat( $angka, $length_decimal );
      } else {
        return angkaRibuan( $angka );
      }
    } else {
      return $default;
    }
